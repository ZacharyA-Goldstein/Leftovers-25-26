...
@Autonomous(name = "pathingCloseTestRed")
public class pathingCloseTestRed extends OpMode {
    ...
    public void buildPaths() {
        ...
        // Heading interpolation: 35° → 0° (as specified in trajectory)
        path1 = follower.pathBuilder()
                .addPath(new BezierLine(startpoint, waypoint1))
                .setLinearHeadingInterpolation(Math.toRadians(35), Math.toRadians(0))
                .build();
        
        // Path 2: waypoint1 (97, 88.2, 0°) → waypoint2 (121, 88.2, 0°)
        // Intake will be on during this path
        path2 = follower.pathBuilder()
                .addPath(new BezierLine(waypoint1, waypoint2))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
                .build();
        
        // Path 3: waypoint2 (121, 88.2, 0°) → waypoint1 (97, 88.2, 0°)
        // Return to waypoint1
     ...
     * - Shooter RPM: -2400
     * - Hood position: 0.2255 (old value, mapped to new range)
     */
    private void setupShooting() {
        try {
            telemetry.addLine("🔧 Setting up shooting at end point...");
            
            // Set spinner to encoder position
            if (robot != null && robot.spinner != null) {
                robot.spinner.setTargetPosition(SPINNER_TARGET_POSITION);
                robot.spinner.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                robot.spinner.setPower(0.8);
            ...
            if (shooterMotor != null) {
                ...
                // Convert target RPM (-2400) to ticks per second for velocity control
            ...
            // Set hood position to 0.2255 (old value, mapped to new range)
            if (hoodServo != null) {
