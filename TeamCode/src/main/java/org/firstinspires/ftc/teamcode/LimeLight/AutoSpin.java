 ...
 */
@TeleOp(name = "Auto Spin", group = "Test")
public class AutoSpin extends LinearOpMode {
    ...
    private void searchAndLock() {
        ...
        if (isLocked || searchStopped) {
            try {
                ...
                if (isLocked) {
                    telemetry.addLine("🔒 LOCKED - OpMode Active");
                }
            } catch (Exception e) {
        ...
        try {
            ...
            if (limelight != null) {
                ...
                if (result != null) {
                    ...
                    telemetry.addData("DEBUG hasTag24", hasTag24 ? "YES" : "NO");
                    telemetry.addData("DEBUG ta check", "%.3f > %.3f = %s", ta, MIN_TAG_AREA, (ta > MIN_TAG_AREA) ? "YES" : "NO");
                    telemetry.addData("DEBUG condition", (ta > MIN_TAG_AREA && hasTag24) ? "PASS - ENTERING ALIGNMENT" : "FAIL - SKIPPING ALIGNMENT");
