 ...
 * to auto‑aim at the BLUE goal (blue AprilTag) while you tune distance vs RPM.
 *
 * Gamepad 1 controls:
 ...
 *
 * Limelight:
 * - Configured for BLUE tag ID (TARGET_TAG_ID_BLUE)
 ...
 */
@TeleOp(name = "At Rock Bottom Blue", group = "TeleOp")
public class AtRockBottomBlue extends LinearOpMode {
    ...
    private static final int TARGET_TAG_ID_BLUE = 20;     // BLUE alliance tag
    ...
    public void runOpMode() {
        ...
        telemetry.addData("Status", "At Rock Bottom Blue ready. Press START.");
        telemetry.update();
    ...
    private void initLimelight() {
        ...
        if (limelight != null) {
            limelight.pipelineSwitch(3); // Pipeline 3 for BLUE alliance (matches CrunchBotBlue)
            sleep(500);
            ...
            if (result != null) {
                ...
                if (fiducials != null) {
                    for (com.qualcomm.hardware.limelightvision.LLResultTypes.FiducialResult fid : fiducials) {
                        if (fid.getFiducialId() == TARGET_TAG_ID_BLUE) {
                            hasTag = true;
                ...
                if (hasTag) {
                    try {
                        cachedTagResult = aprilTagDetector.getTagById(TARGET_TAG_ID_BLUE);
                        if (cachedTagResult != null && cachedTagResult.isValid) {
                            ...
                            cachedTagResult.tagId = TARGET_TAG_ID_BLUE;
    ...
    private void updateTelemetry() {
        telemetry.addLine("=== At Rock Bottom Blue ===");
