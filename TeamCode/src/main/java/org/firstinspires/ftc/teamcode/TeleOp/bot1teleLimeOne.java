...
public class bot1teleLimeOne extends LinearOpMode {
    ...
    // Spinner step (angle adjust) flags (left/right disabled; up/down for transfer control)
        ...
        while (opModeIsActive()) {
            ...
            if (currentIntakeTrigger && !lastIntakeTrigger) {
                ...
                if (intakeOn) {
                    ...
                    // Run intake wheel briefly to push first ball into lift
                    // After first ball is in, gate closes and other balls wait in tunnel
                    if (intakeWheel != null) {
            ...
            // Lift manual control with D-pad up/down (only when auto-aim is off)
